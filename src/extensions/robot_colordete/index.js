...
class RobotColordete {
  ...
  getInfo() {
    ...
    return {
      id: 'robotcolordete',
      name:  formatMessage({
            id: 'robotcolordete.name',
            default: 'Color Detection',
            description: 'robotcolordete.name'
        }),
      color1: '#b22222',
      menuIconURI: imgIcon,
      ...
      blocks: [
...

        {
            ...
            text: formatMessage({
                id: 'robotcolordete.readColor',
                default: 'Get color block [THREE] value',
                description: 'robotcolordete.readColor'
            }),
            arguments:{
                THREE:{
    ...
    showToast(message, duration = 3000) {
        ...
        if(message == "请先开启摄像头"){
            ...
            toast.textContent = formatMessage({
                id: 'robotcolordete.showToast.firstMode',
                default: 'Color detection mode is not enabled',
                description: 'robotcolordete.showToast.firstMode'
            })
        }
       ...
       if(this.mode){
        ...
        if(!imageLoad.getColorReco()){
            ...
            if(imageLoad.getCameraPlace()=='0'){
                ...
                await new Promise(resolve => setTimeout(resolve, 1000)); 
                imageLoad.getCamObj().startColorDetection()
            }
    ...
    async cstopMode(args){
        if(this.mode){
            if(imageLoad.getColorReco()){
                // if(imageLoad.getModelOpen().playground=='colordete'){
                //     await this.runtime.ioDevices.video.stopColorDetection()
                // }else if(imageLoad.getModelOpen().dialogCam=='colordete'){
                //     imageLoad.getCamObj().stopColorDetection()
                // }
                try{
                    await this.runtime.ioDevices.video.stopColorDetection()
                }catch(e){
