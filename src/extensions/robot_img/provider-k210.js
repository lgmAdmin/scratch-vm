...
class Providerk210 {
     ...
     */
    getFrame ({
        dimensions = Providerk210.DIMENSIONS,
        mirror = this.mirror,
        format = Providerk210.FORMAT_IMAGE_DATA,
        ...
        try{
            ...
            if (format === Providerk210.FORMAT_IMAGE_DATA) {
                return this._context.getImageData(0, 0, width, height);
            } else if (format === Providerk210.FORMAT_CANVAS) {
                return this._canvas;
            }
    ...
    async _setupImageStream () {
        ...
        // img.src = `http://${this._ip}:81/stream?ts=${Date.now()}`;  // Set the image feed URL here
        // 设置 6 秒超时检测
        ...
        this._img.onload = () => {
            ...
            imageLoad.setPlayGround('netImg')
            // Create a canvas to render the image frames
            this._canvas = document.createElement('canvas');
            this._canvas.width = Providerk210.DIMENSIONS[0];
            this._canvas.height = Providerk210.DIMENSIONS[1];
