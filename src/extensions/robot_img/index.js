...
class RobotImg {
    constructor(runtime){
        ...
        this.closeModeChannel.addEventListener('message',async(event)=>{
            ...
            cam.stopAprilTag()
            cam.stopQRDetection()
            cam.stopColorDetection()
            cam.stopColorPlaceDetection()
            cam.stopWColorBlockDetection()
            cam.stopWItem()
        ...
        this.channelOpenCamera = new BroadcastChannel('open-camera')
        this.channelOpenCamera.addEventListener('message',(event)=>{
            if(event.data){
                this.onlyOpenCamera()
            }else{
                this.onlyCloseCamera()
            }
        })

        this.channelCloseDialog = new BroadcastChannel('close-dialog')
        this.channelCloseDialog.addEventListener('message',(event)=>{
            if(imageLoad.getWhatCamera().dialogCam=='robotImg'){
                this.cstartCamera({ONE:'2'})
            }else if(imageLoad.getWhatCamera().dialogCam=='computer'){
                this.cstartComputerCamera({ONE:'2'})
            }else if(imageLoad.getWhatCamera().dialogCam=='netImg'){
                this.cstartNetCamera({ONE:'2'})
            }
            cam.stopAprilTag()
            cam.stopQRDetection()
    ...
    return {
      ...
      blocks: [

         {
            ...
            // text: '设置摄像头画面[ONE]',
             text: formatMessage({
                id: 'robotimg.howStartCamera',
                default: 'Set camera display [ONE]',
                description: 'robotimg.howStartCamera'
            }),
            arguments:{
                ONE:{
        ...
        },
        {
            ...
            // text: '[ONE]机器人摄像头',
            text: formatMessage({
                id: 'robotimg.cstartCamera',
                default: '[ONE] robot camera',
                description: 'robotimg.cstartCamera'
            }),
            arguments:{
                ONE:{
...

        {
            ...
            // text: '[ONE] 摄像头 [TWO]',
            text: formatMessage({
                id: 'robotimg.cstartComputerCamera',
                default: '[ONE] camera [TWO]',
                description: 'robotimg.cstartComputerCamera'
            }),
            arguments:{
                ONE:{
...

        {
            ...
            // text: '[ONE]摄像头 IP:[TWO]',
             text: formatMessage({
                id: 'robotimg.cstartNetCamera',
                default: '[ONE] camera IP:[TWO]',
                description: 'robotimg.cstartNetCamera'
            }),
            arguments:{
                ONE:{
...

        {
            ...
            // text: '机器人摄像头开启成功？',
            text: formatMessage({
                id: 'robotimg.isOpenCamera',
                default: 'Robot camera enabled?',
                description: 'robotimg.isOpenCamera'
            }),
            arguments:{
...

        {
            ...
            // text: '模型加载成功？',
            text: formatMessage({
                id: 'robotimg.isOpenModel',
                default: 'Model loaded successfully?',
                description: 'robotimg.isOpenModel'
            }),
            arguments:{
      ...
      menus:{
        ...
        CAMERA_PLACE:{
            ...
            items: [
                {
                    text: formatMessage({
                        id: 'robotimg.menuCameraPlace.playground',
                        default: 'Stage',
                        description: 'robotimg.menuCameraPlace.playground'
                    }),
                    value: '0'
                },
                {
                    text: formatMessage({
                        id: 'robotimg.menuCameraPlace.dialog',
                        default: 'Popup',
                        description: 'robotimg.menuCameraPlace.dialog'
                    }),
                    value: '1'
                },
        ...
        MENU_MIRROR:{
            ...
            items: [
                {
                    text: formatMessage({
                        id: 'robotimg.menuMirror.start',
                        default: 'On',
                        description: 'robotimg.menuMirror.start'
                    }),
                    value: '0'
                },
                {
                    text: formatMessage({
                        id: 'robotimg.menuMirror.mirrorstart',
                        default: 'Mirror On',
                        description: 'robotimg.menuMirror.mirrorstart'
                    }),
                    value: '1'
                },
                {
                    text: formatMessage({
                        id: 'robotimg.menuMirror.stop',
                        default: 'Off',
                        description: 'robotimg.menuMirror.stop'
                    }),
                    value: '2'
                },
...
//   }
   ...
   async waitForSuccessTotal() {
    ...
    howStartCamera(args){
        imageLoad.setCameraPlace(args.ONE)
    }
    ...
    async onlyOpenCamera(){
        ...
        if(socket.checkWebSocketStatus()==4 || socket.checkWebSocketStatus()==0){
            ...
            if (!result) {
                ...
                // 第二次等待响应，但不判断
                const result2 = await this.waitForSuccess(3000);

                if(result2) this.channelOpenCamera.postMessage(socket.getIp())
                
            }else{
                this.channelOpenCamera.postMessage(socket.getIp())
            }
    ...
    async onlyCloseCamera(){
        ...
        await this.waitForSuccessTotal()
    }
    async cstartCamera(args){
        if(this.mode){
            if(socket.getIp().length==0){
            ...
            if(args.ONE!='2'){
                ...
                if(!imageLoad.getIsImage()){
                    ...
                    if(imageLoad.getCameraPlace() == '0' && imageLoad.getWhatCamera().playground=='robotImg'){
                        if(args.ONE=='0'){
                            this.runtime.ioDevices.video.mirror=false
                        }else{
                            this.runtime.ioDevices.video.mirror=true
                        ...
                        await new Promise(resolve => setTimeout(resolve, 2000)); 
                    }else if(imageLoad.getCameraPlace() == '0' && imageLoad.getWhatCamera().dialogCam=='robotImg'){
                    ...
                    }else if(imageLoad.getCameraPlace() == '1' && imageLoad.getWhatCamera().playground=='robotImg'){
                        await new Promise(resolve => setTimeout(resolve, 50));  
                        this.stopAllMode()
                ...
                if(this.mode){
                    ...
                    if(imageLoad.getWhatCamera().playground=='robotImg'){
                        await new Promise(resolve => setTimeout(resolve, 50));  
                        this.stopAllMode()
                        await new Promise(resolve => setTimeout(resolve, 200));  
                        this.runtime.ioDevices.video.disableVideo();
                        ...
                        imageLoad.setPlayGround('')
                        this.isCamera=false
                    }else if(imageLoad.getWhatCamera().dialogCam=='robotImg'){
                        await new Promise(resolve => setTimeout(resolve, 50));  
                        cam.close()
                        imageLoad.setDialog('')
                        imageLoad.setIsImage(false)
    ...
    getCamera(){
        let item=[{
            text: formatMessage({
                id: 'robotimg.getCameraDefault',
                default: 'Please select',
                description: 'robotimg.getCameraDefault'
            }),
            value:'请选择'
        }];
    ...
    async cstartComputerCamera(args){
        ...
        if(args.ONE!='2'){
            ...
            if(imageLoad.getComputer()){
                if(imageLoad.getWhatCamera().playground.length>0){
                     try{
                        await this.cstopMode({ONE:'3'})
                        await this.cstopMode({ONE:'4'})
                        await this.cstopMode({ONE:'6'})
                ...
                if(imageLoad.getWhatCamera().playground=='computer'){
                    this.stopAllMode()
                    await new Promise(resolve => setTimeout(resolve, 200));  
                    this.runtime.ioDevices.video.disableVideo();
                    imageLoad.setPlayGround('')
            ...
            if(args.TWO!='请选择'){
                if(imageLoad.getCameraPlace()=='0'){
                    ...
                    this.runtime.ioDevices.video.enableVideo()
                    imageLoad.setComputer(true) 
                    imageLoad.setPlayGround('computer')
    ...
    async cstartNetCamera(args){
        if(args.ONE!='2'){
            ...
            // }
                ...
                if((imageLoad.getCameraPlace()=='0' && imageLoad.getWhatCamera().playground.length>0 &&imageLoad.getWhatCamera().playground!='netImg') || (imageLoad.getCameraPlace()=='1' &&imageLoad.getWhatCamera().dialogCam.length>0 && imageLoad.getWhatCamera().dialogCam!='netImg')){
                    this.showToast('当前模式已有摄像头占用')
                    return
                }
                // if(imageLoad.getIsImage() || imageLoad.getComputer()){
                //     return 
                // }
                if(imageLoad.getIsK210()) {
                    if(imageLoad.getCameraPlace() == '0' && imageLoad.getWhatCamera().playground=='netImg'){
                        if(args.ONE=='0'){
                            this.runtime.ioDevices.video.mirror=false
                            this.stopAllMode()
                            await new Promise(resolve => setTimeout(resolve, 200));  
                    ...
                    }else if(imageLoad.getCameraPlace() == '1' && imageLoad.getWhatCamera().dialogCam=='netImg'){
                        if(args.ONE=='0'){
                            cam.setMirror(false)
                            cam.close()
                            imageLoad.setDialog('')
                    ...
                    }else if(imageLoad.getCameraPlace() == '0' && imageLoad.getWhatCamera().dialogCam=='netImg'){
                    ...
                    }else if(imageLoad.getCameraPlace() == '1' && imageLoad.getWhatCamera().playground=='netImg'){
                        this.stopAllMode()
                        await new Promise(resolve => setTimeout(resolve, 200));  
                        this.runtime.ioDevices.video.disableVideo();
                        imageLoad.setPlayGround('')
            ...
            if(this.mode){
                await new Promise(resolve => setTimeout(resolve, 50));  
                if(imageLoad.getWhatCamera().playground=='netImg'){
                    this.stopAllMode()
                    await new Promise(resolve => setTimeout(resolve, 200));  
                    this.runtime.ioDevices.video.disableVideo();
                    imageLoad.setPlayGround('')
